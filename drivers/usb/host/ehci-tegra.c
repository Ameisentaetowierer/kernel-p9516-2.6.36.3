...
//zhaoq
struct tegra_ehci_hcd * g_tegra_hcd[2];
...
//Compal Start William 2011-08-26, add zhaoqian's USB3(Host) docking patch
//zhaoq store hcd struct
	g_tegra_hcd[instance-1] = tegra;
//Compal End William 2011-08-26
...
extern int usb_autosuspend_device(struct usb_device *udev);
//zhaoq
void enable_usb3 (void)
{
	struct tegra_ehci_hcd *tegra = g_tegra_hcd[1];
	struct usb_hcd *hcd = ehci_to_hcd(tegra->ehci);
	struct usb_device *udev = hcd->self.root_hub;
	...
	printk(" *** enable usb3, hub%d\n",hcd->self.busnum);
}
EXPORT_SYMBOL(enable_usb3);
void disable_usb3 (void)
{
	...
	struct usb_device *udev = hcd->self.root_hub;
	usb_lock_device(udev);
	udev->autosuspend_delay = 2* HZ;
	usb_autosuspend_device(udev);
	printk(" *** disable usb3, hub%d\n",hcd->self.busnum);
	usb_unlock_device(udev);
}
EXPORT_SYMBOL(disable_usb3);
//Compal End William 2011-08-26
...
static int tegra_ehci_resume(struct platform_device *pdev)
{
	...
	if ((tegra->bus_suspended) && (tegra->power_down_on_bus_suspend))
		return 0;
//Compal Start William 2011-08-26, add zhaoqian's USB3(Host) docking patch
/*zhaoq, make sure tegra->power_down_on_bus_suspend is one after suspend.
we can leave usb3 runing on first boot, but after suspend, should be stop.
triger by dock plug in to run or stop */
	if (tegra->power_down_on_bus_suspend == 0 )
		tegra->power_down_on_bus_suspend = 1;
//zhaoq end
//Compal End William 2011-08-26
