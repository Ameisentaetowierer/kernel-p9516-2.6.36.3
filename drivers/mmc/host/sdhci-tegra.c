...
#include <linux/timer.h>
	...
	struct sdhci_host *sd_sdhost;
	struct tegra_sdhci_host *sd_host;
	int card_present;
#define TEGRA_GPIO_PI5          69
#define DEBOUNCE_TIME		2000
//

static void sd_debounce_timer_func(unsigned long unused)
{
	int cp = gpio_get_value(sd_host->cd_gpio);
	if( cp == card_present ){
		sd_host->card_present =
                        (gpio_get_value(sd_host->cd_gpio) == sd_host->cd_gpio_polarity);
                tasklet_schedule(&sd_sdhost->card_tasklet);
	}
}
...
//Charles
	card_present = gpio_get_value(host->cd_gpio);
	
	printk("carddetect_irq, cd:%d\n", card_present);

	if( host->cd_gpio == TEGRA_GPIO_PI5 && !card_present){
		card_present = gpio_get_value(host->cd_gpio);
...
//Charles
	setup_timer(&debounce_timer, sd_debounce_timer_func, 0);
	if( host->cd_gpio == TEGRA_GPIO_PI5 ){
		sd_sdhost = sdhci;
		sd_host = host;
	}
//
