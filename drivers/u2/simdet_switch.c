...
#define SIMDET_DEBOUNCE_TIME_MS 200
...
static void simdet_switch_work(struct work_struct *work)
{
    ...
    sim_Status = gpio_get_value(pSwitch->gpio);
    printk("%s: Sim status is set\n", DRIVER_NAME);
    switch_set_state(&pSwitch->sdev, sim_Status);

    if((!initialized) && (sim_Status == 0)) {
	// Manually enable 3G power to speed up booting time
	enable_ThreeGPower();
	printk("%s: Enable power for 3G module\n", DRIVER_NAME);
	initialized = true;
    }
}
...
{
    struct simdet_switch_data *pSwitch = (struct docking_switch_data *)dev_id;

    mdelay(SIMDET_DEBOUNCE_TIME_MS);
