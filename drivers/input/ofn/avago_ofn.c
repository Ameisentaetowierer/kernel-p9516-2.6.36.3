...
int inline ofn_chk_id( struct avago_ofn_data *ts )
{
   return ((avago_read_register_byte(ts->spi, 0)==AVAGO_PRODUCT_ID)?OFN_GOOD:OFN_ERR);
}
...
static int __devinit avago_ofn_probe(struct spi_device *spi)
{
    ...
    {
        pr_info("%s success to get regulator LDO5\n", __func__);
    }
...
err_gpio:
    ...
    //[ASD2_ES1|Jaco_Gao|20110630] re-enable interrupt of motion pin after chip initialization
    set_irq_type(gpio_to_irq(ts->irq), IRQF_TRIGGER_FALLING);
    set_irq_type(gpio_to_irq(ts->dome_gpio), IRQF_TRIGGER_FALLING); 
