		    ...
		    struct spi_transfer *t, bool is_first_of_msg)
{
    ...
    //+++[ASD2_ES1|Jaco_Gao|2011.06.24] Add for fitting ofn spi bus requiement
    if( t->rx_buf )
    {
        // guard time between two packet
        //val &= ~(SLINK_INT_SIZE(~0)|SLINK_SS_SETUP(~0)|SLINK_WAIT_PACK_INT(~0));
        val &= ~(SLINK_INT_SIZE(~0)|SLINK_WAIT_PACK_INT(~0));

        // @1MHz : size 4 is about 1us gap
        // using 5us gard time between packet, size 20
        val |= SLINK_INT_SIZE(20);
        //val |= SLINK_SS_SETUP(3);
        //val |= SLINK_WAIT_PACK_INT(0x7);
    }
    //---[ASD2_ES1|Jaco_Gao|2011.06.24] Add for fitting ofn spi bus requiement
...
struct spi_tegra_data *gtspi = NULL;

void spi_err_handle( void )
{
    struct spi_tegra_data *tspi = gtspi;
    ...
    // isr_thread
    handle_cpu_based_xfer((void*)tspi);

}

void spi_reset_clk( void )
{
    struct spi_tegra_data *tspi = gtspi;
