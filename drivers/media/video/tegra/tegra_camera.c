...
/* COMPAL - START - [2011/8/25] - Sandow - [Camera]: add system node for debug   */
static ssize_t ldo6_show(struct kobject *kobj, struct kobj_attribute *attr, char * buf)
{
    return (sizeof(buf));
}
static ssize_t ldo6_store(struct device *dev, struct device_attribute *attr,
		const char *buf, size_t count)
{
	 ...
	 if (*buf == '1')
	 {
		...
		if (!ret)
			printk("[regulator]: power on LDO6 \n");
	 }
	 if (*buf == '0')
	 {
		ret = regulator_disable(tegra_camera_regulator_csi);
		if (!ret)
			printk("[regulator]: power off LDO6 \n");
	 }
	 return count;
}

static struct kobject *ldo6debug_kobj;
...
static int tegra_camera_probe(struct platform_device *pdev)
{
   ...
   /* COMPAL - START - [2011/8/25  16:11] - Sandow - [Camera]: add system node for debug   */
 	ldo6debug_kobj = kobject_create_and_add("Regulator", NULL);
    	if (ldo6debug_kobj == NULL) {
       	 printk("%s: subsystem_register failed\n", __FUNCTION__);
    	}
    	err = sysfs_create_group(ldo6debug_kobj, &attr_group);
    	if(err) {
