...
//compal start willy
		...
		//Sam dmic
		snd_soc_write(codec, WM8903_CONTROL_INTERFACE_TEST_1, 0x0003);
		snd_soc_write(codec, WM8903_MIC_BIAS_CONTROL_0, 0x17);
                snd_soc_write(codec, WM8903_CLOCK_RATES_1, 0x0c07);
                snd_soc_write(codec, WM8903_DIGITAL_MICROPHONE_0, 0x0173);
                snd_soc_write(codec, WM8903_GPIO_CONTROL_2, 0x06a0);
                snd_soc_write(codec, WM8903_GPIO_CONTROL_1, 0x0620);
		//use dmic = 0x2a38 amic = 0x2838
		//Webber set AMIC DMIC switch
                ...
                if(global_action != 0)
                {
                        for(i=1;i<=10;i++){
                                ...
                                msleep(5);
                                mic_status = snd_soc_read(codec, WM8903_INTERRUPT_STATUS_1);
                                snd_soc_update_bits(codec, WM8903_INTERRUPT_POLARITY_1,
                                        (WM8903_MICDET_INV | WM8903_MICSHRT_INV),~(WM8903_MICDET_INV | WM8903_MICSHRT_INV));
                                msleep(5);
                                mic_status = ( snd_soc_read(codec, WM8903_INTERRUPT_STATUS_1) & 0xf000 ) >> 12;
                                if(mic_status & 0xc)
                                if(++mic_count > 6)
                                        break;
			}
                ...
                if(global_action != 2)
		{
                        snd_soc_write(codec, WM8903_CLOCK_RATE_TEST_4, 0x2a38);
			snd_soc_write(codec, WM8903_MIC_BIAS_CONTROL_0, 0x16);
		}
		else
			snd_soc_write(codec, WM8903_CLOCK_RATE_TEST_4, 0x2838);
                snd_soc_write(codec, WM8903_ADC_DIGITAL_VOLUME_LEFT, 0x00ef);
                snd_soc_write(codec, WM8903_ADC_DIGITAL_VOLUME_RIGHT, 0x01ef);
	}
...
//compal start willy
	pr_debug("tegra_wired_jack_conf.en_mic_int = %d \n",tegra_wired_jack_conf.en_mic_int);
	pr_debug("tegra_wired_jack_conf.en_mic_ext = %d \n",tegra_wired_jack_conf.en_mic_ext);
	pr_debug("event = %d \n",event);
//compal end willy
	return 0;
}
